...
class SelectionListAdapter(private val id: String) : RecyclerView.Adapter<SelectionListAdapter.SelectionViewHolder>() {
    ...
    var clickListener: ((SelectionModel) -> Unit)? = null
        ...
        fun bind(item: SelectionModel) {
            when (id) {
                TYPE_OVD -> {
                    ...
                    sb.append(item.title).append(" - ").append(item.value)
                    titleTextView.text = sb.toString()
                }
                TYPE_OVZ -> {
    ...
    companion object {
        ...
        private const val TYPE_OVD = "ovd"
        private const val TYPE_OVZ = "ovz"
    }
}